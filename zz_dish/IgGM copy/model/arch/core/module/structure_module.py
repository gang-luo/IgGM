...
class StructureModule(nn.Module):
    ...
    def forward(
        ...
        # ====== 新增：解包 FR-trsl EDM 系数并缩放 trsl_xt ======
        fr_c_in   = region_metadata['anchor_frame_meta']['fr_c_in'].to(device=device, dtype=dtype)    # scalar
        fr_c_skip = region_metadata['anchor_frame_meta']['fr_c_skip'].to(device=device, dtype=dtype)  # scalar
        fr_c_out  = region_metadata['anchor_frame_meta']['fr_c_out'].to(device=device, dtype=dtype)   # scalar

        # 对 trsl_xt 做 c_in 归一化，使输入方差 ≈ O(1)
        # 形状 [B, 3]；broadcast 用 .view(-1,1)
        trsl_xt0_scaled = trsl_xt0 * fr_c_in.view(-1, 1) if fr_c_in.numel() > 1 \
                        else trsl_xt0 * fr_c_in  # scalar 广播
        # ====== 新增结束 ======
        ...
        for layer_idx in range(n_lyrs):
            ...
            fr_out = self.net['fr_branch'](
                ...
                rota_xt=rota_xt_in,
                trsl_xt_scaled=trsl_xt0_scaled,   # ← 改名：c_in 缩放后的输入
                trsl_xt_orig=trsl_xt0,            # ← 新增：原始带噪 trsl，用于 c_skip 组
                antibody_local_coords=antibody_local_coords,
                fr_sigma_trsl=fr_sigma_trsl,
                fr_sigma_rota=fr_sigma_rota,
                fr_c_skip=fr_c_skip,              # ← 新增
                fr_c_out=fr_c_out,                # ← 新增
            )
            ...
            # 5.dta亲和力预测，用abagker重新训练一个模型，用于监督预测。
