...
class FRBranch(nn.Module):
    ...
    def __init__(self, c_s: int = 384, c_e: int = 64, c_hidden: int = 384) -> None:
        ...
        self.linear_t = nn.Linear(c_hidden + 3, 3)   # 与之前一致
        self.linear_q = nn.Linear(c_hidden + 3, 3)   # ← 同样 +3
        self.delta_feat = nn.Linear(c_hidden, c_s)

        nn.init.normal_(self.linear_t.weight)#, std=1e-1
        nn.init.zeros_(self.linear_t.bias)
        nn.init.normal_(self.linear_q.weight)#, std=1e-1
    ...
    #     c_skip_trsl: torch.Tensor | None = None, 
    #     c_out_trsl: torch.Tensor | None = None,
    # ) -> dict:
    #     if rota_xt.ndim == 2:
    ...
    #     if c_skip_trsl is not None and c_out_trsl is not None:
    #         # 2. 转换 EDM 权重类型以匹配当前计算
    #         c_skip_t = c_skip_trsl.to(pooled.dtype)
    #         c_out_t = c_out_trsl.to(pooled.dtype)
            
    #         # 3. 将网络预测的局部残差向量 (F_theta) 旋转到全局坐标系
    #         # （因为 trsl_xt 是在全局下的）
    #         F_theta_global = raw_delta_trsl @ rota_xt.transpose(-1, -2)
            
    #         # 4. EDM 奇迹公式：c_skip 衰减掉高噪音，c_out 放大预测值！
    #         updated_trsl = c_skip_t * trsl_xt + c_out_t * F_theta_global
            
    #         # 5. 反推供输出记录的中间变量
    #         delta_trsl_global = updated_trsl - trsl_xt
    #         delta_trsl_local = delta_trsl_global @ rota_xt
    #     else:
    #         # 兼容老逻辑的保护分支
    ...
    #     # ==============================================================
    #     # 旋转 (Rotation) 保持原样！(因为 R 矩阵在流形上，不能做 c_skip 线性插值)
    #     # ==============================================================
    #     sigma_rota_expand = fr_sigma_rota.view(-1, 1).to(pooled.dtype)
    #     raw_delta_rota = self.linear_q(pooled) * 2.0  # 稍微放大让旋转更灵敏
    ...
    def forward(
            ...
            curr_coords: torch.Tensor,
            rota_xt: torch.Tensor,
            trsl_xt_scaled: torch.Tensor,      # [B,3] c_in 归一化后，方差 O(1)，拼入网络
            trsl_xt_orig: torch.Tensor,         # [B,3] 原始带噪平移，用于 c_skip 组装
            antibody_local_coords: torch.Tensor,
            fr_sigma_trsl: torch.Tensor,
            fr_sigma_rota: torch.Tensor | None = None,
            fr_c_skip: torch.Tensor | None = None,   # [B] or scalar
            fr_c_out:  torch.Tensor | None = None,   # [B] or scalar
        ) -> dict:
            # --- shape 规范化 ---
            if rota_xt.ndim == 2:
                rota_xt = rota_xt.unsqueeze(0)
            ...
            if trsl_xt_orig.ndim == 1:
                trsl_xt_orig = trsl_xt_orig.unsqueeze(0)
            if antibody_local_coords.ndim == 3:
                antibody_local_coords = antibody_local_coords.unsqueeze(0)
            if antibody_mask.ndim == 1:
                antibody_mask = antibody_mask.unsqueeze(0)
            ...
            # =========================================================
            # 平移量：EDM preconditioning（正确版）
            #
            # 核心：把 trsl_xt_scaled 拼到 pooled 后，网络才能"看到"
            # 当前带噪平移状态，F_θ 预测归一化空间中的 x0
            # =========================================================
            trsl_xt_scaled = trsl_xt_scaled.to(dtype=pooled.dtype)
            pooled_with_trsl = torch.cat([pooled, trsl_xt_scaled], dim=-1)  # [B, c_hidden+3]
            F_theta_trsl = self.linear_t(pooled_with_trsl)                   # [B, 3]，归一化空间

            # 组装 x0 预测：c_skip · xt_orig + c_out · F_θ
            # c_skip 接近1 → 小噪声时几乎直接返回 xt（几乎干净）
            # c_out  接近0 → 小噪声时网络修正幅度小，训练信号稳定
            c_s = fr_c_skip.view(-1, 1).to(dtype=pooled.dtype)   # [B,1]
            c_o = fr_c_out.view(-1, 1).to(dtype=pooled.dtype)    # [B,1]

            trsl_x0_pred = c_s * trsl_xt_orig + c_o * F_theta_trsl   # [B,3]，单位 Å


            rota_xt_log = skew2vec(log_rmat(rota_xt)).to(dtype=pooled.dtype)   # [B, 3]                      # [B, 3]，李代数表示
            rota_xt_log = rota_xt_log.to(dtype=pooled.dtype)
            pooled_with_rota = torch.cat([pooled, rota_xt_log], dim=-1)  # [B, c_hidden+3]
            raw_delta_rota   = self.linear_q(pooled_with_rota)           # [B, 3]
            ...
            return {
                ...
                'trsl':           trsl_x0_pred,       # 语义：对 trsl_0 的直接预测
                'rota':           updated_rota,
                'mask':           (denom.squeeze(-1) > 0).to(torch.bool),
                'F_theta_trsl':   F_theta_trsl,        # 网络原始输出（归一化空间，调试用）
                'raw_delta_trsl': F_theta_trsl,        # 兼容旧字段名
                'delta_trsl':     trsl_x0_pred - trsl_xt_orig,  # 实际位移，tensorboard 用
            }
...
class CDRFusionBlock(nn.Module):
    ...
    def forward(
        ...
        *,
        ...
        pred_x0_local = v * loop_xt_local_orig + c_out * F_theta
