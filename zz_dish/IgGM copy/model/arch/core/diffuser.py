...
class Diffuser:
    ...
    def __init__(
        ...
        # --- 修正: 适配 3D 分子的 EDM 连续时间调度 ---
        self.sigma_min = 0.05  # 稍微提高底线，防止由于精度问题产生的数值不稳定
        
        # 最大噪声 80 埃其实太大了！一个抗体复合物的直径通常只有几十埃。
        # 50 埃已经足够把结构完全打散到认不出来了。过大的噪声毫无意义，只会徒增优化难度。
        self.sigma_max = 50.0  
        
        # 【关键修改】：降低 rho 的值！
        # 将 rho 从 7.0 降到 3.0 或 4.0。这会让中间时间步的噪声分布更加均匀，
        # 避免出现 80步只有2埃，而 180步直接飙升到50埃的断崖式跳跃。
        self.rho = 3.0
    ...
    def _run_fr_cdr_sync(self, prot_data_orig, idxs_step):
        ...
        # ====== 新增：为 FR 平移量计算 EDM preconditioning 系数 ======
        # 与 CDR 使用完全相同的 sigma_data = 4.0，保证两支路对齐
        
        sigma_t = self.sigmas[idxs_step].to(device=device, dtype=dtype)
        _sigma_data_fr = torch.as_tensor(20.0, device=device, dtype=dtype)
        _sigma_t_fr = sigma_t  # sigma_t 已由 self.sigmas[idxs_step] 算出
        
        _denom_fr = torch.sqrt(_sigma_t_fr ** 2 + _sigma_data_fr ** 2)

        fr_c_in   = (1.0 / _denom_fr)                                          # scalar
        fr_c_skip = (_sigma_data_fr ** 2 / (_sigma_t_fr ** 2 + _sigma_data_fr ** 2))  # scalar
        fr_c_out  = (_sigma_t_fr * _sigma_data_fr / _denom_fr)                 # scalar
        ...
        prot_data_pert = {
            ...
            "anchor_frame_meta": {
                ...
                # ====== 新增三行 ======
                "fr_c_in":       fr_c_in.detach().clone(),    # scalar tensor
                "fr_c_skip":     fr_c_skip.detach().clone(),  # scalar tensor
                "fr_c_out":      fr_c_out.detach().clone(),   # scalar tensor
            },
