...
class _ProteinSampleDataset(Dataset):
    ...
    def _apply_random_rotation(self, payload: Dict[str, Any]) -> Dict[str, Any]:
        """纯 PyTorch 张量加速版的 3D 随机旋转增强"""
        
        prot = payload["prot_data_curr"]
        cord_ref = prot["cord"]
        device, dtype = cord_ref.device, cord_ref.dtype
        
        # 1. 纯张量化生成均匀 SO(3) 旋转矩阵 (Shoemake 算法)
        # 一次性生成 3 个 [0, 1) 的随机数，全部在对应的 device 上，避免数据搬运
        u = torch.rand(3, device=device, dtype=dtype)
        u1, u2, u3 = u[0], u[1], u[2]
        
        pi2 = 2.0 * torch.pi
        w = torch.sqrt(1.0 - u1) * torch.sin(pi2 * u2)
        x = torch.sqrt(1.0 - u1) * torch.cos(pi2 * u2)
        y = torch.sqrt(u1) * torch.sin(pi2 * u3)
        z = torch.sqrt(u1) * torch.cos(pi2 * u3)
        
        # 组装 3x3 旋转矩阵 (纯张量拼接，速度极快)
        R = torch.stack([
            torch.stack([1.0 - 2*y*y - 2*z*z,  2*x*y - 2*z*w,      2*x*z + 2*y*w]),
            torch.stack([2*x*y + 2*z*w,      1.0 - 2*x*x - 2*z*z,  2*y*z - 2*x*w]),
            torch.stack([2*x*z - 2*y*w,      2*y*z + 2*x*w,      1.0 - 2*x*x - 2*y*y])
        ])
        
        # 2. 浅拷贝，防止污染 DataLoader 的 Cache
        new_payload = dict(payload)
        new_prot = dict(prot)
        new_payload["prot_data_curr"] = new_prot
        
        # 3. 批量矩阵乘法 (Batch Matmul) 瞬间完成所有坐标的旋转
        # 无论是 Nx3 (原子) 还是 Nx14x3 (全原子组)，通过 view(-1, 3) 拍扁后一次性乘完，再变回原状
        R_T = R.T  # 预先转置，用于右乘坐标矩阵
        
        for key in ["cord", "cords_atom14", "a-cord"]:
            if key in new_prot and torch.is_tensor(new_prot[key]):
                orig_shape = new_prot[key].shape
                # 张量底层 C++ 运算：[M, 3] @ [3, 3] -> [M, 3]
                new_prot[key] = torch.matmul(new_prot[key].view(-1, 3), R_T).view(orig_shape)
    ...
    def __getitem__(self, index):
        ...
        for _ in range(10): # 重复尝试返回结果
            ...
            try:
                ...
                if getattr(self, "use_random_rotation", False):
                    payload = self._apply_random_rotation(payload)
                out = {
                    "idx_step": step,
...
class ProcessedSabdabDataModule(pl.LightningDataModule):
    ...
    def _make_dataset(self, items: List[Dict[str, object]], subset_name: Optional[str] = None, is_train: bool = False) -> Dataset:
    ...
    def setup(self, stage: Optional[str] = None):
        ...
        self.train_ds = self._make_dataset(train_items,is_train=True)
