...
class IgGMPaperLoss:
    ...
    def __call__(self, inputs: Dict[str, torch.Tensor], outputs: Dict[str, torch.Tensor]) -> Dict[str, torch.Tensor]:
        ...
        if self.cfg.loss_mode in {'fr_cdr_boltz', 'fr_cdr_iggm', 'boltz_style', 'iggm_style', 'aligned_backbone_cdr_vio'}:
    ...
    def _openfold_violation_loss(self, pred: torch.Tensor, atom_exists: torch.Tensor, seq_o) -> torch.Tensor:
        try:
            from openfold.utils.loss import find_structural_violations, violation_loss  # type: ignore
        except ImportError:
            try:
                # Support vendored/forked layout requested by local environment.
                from loss.openfold.utils.loss import find_structural_violations, violation_loss  # type: ignore
            except ImportError as exc:  # pragma: no cover
                raise ImportError(
                    "OpenFold violation functions are required but not importable from "
                    "`openfold.utils.loss` or `loss.openfold.utils.loss`."
                ) from exc
    ...
    def _aligned_backbone_cdr_vio_loss(self, inputs: Dict[str, torch.Tensor], outputs: Dict[str, torch.Tensor]) -> Dict[str, torch.Tensor]:
        ...
        loss_vio = self._openfold_violation_loss(pred_aligned, atom_mask.to(pred.dtype), inputs["seq-o"])
