...
- Enabled checkpointing for both Evoformer and StructureModule (`enable_structure_checkpoint`) from Lightning, reducing AF2-style structure stack activation memory.

### 3) Avoid retaining non-essential train-time auxiliaries
- Added `return_full_aux` path in DesignModel/StructureModule. Train steps now skip storing heavy `param_list` and `plddt_list`, while val/test keep full outputs.

### 4) Lower-memory loss computation
...
### 6) Graceful OOM and explicit length handling
...
- Added featurization OOM fallback (`featurizer_cpu_offload_on_oom`): retries `_build_inputs_cm` with temporary PLM CPU offload when GPU featurization spikes.
- Added optional OOM skip behavior (`skip_oom_batch`) with cache cleanup (`clear_cache_on_oom`).

### 7) Memory diagnostics instrumentation
...
- `memory.auto_chunk_on_oom: true`, `memory.min_chunk_size: 2`
- `memory.featurizer_cpu_offload_on_oom: true`
