...
class FRBranch(nn.Module):
    def __init__(
        ...
        rota_geom_dim = c_pose + pose_heads * 3
        ...
        pose_token_dim = c_pose + pose_heads * 3 + rbf_bins
        ...
        trsl_input_dim = c_hidden + 3 + 32 + self.iface_dim + c_pose
        rota_input_dim = c_hidden + 32 + c_pose
    ...
    def forward(
        ...
        interface_feature = self._interface_pool(
            res_hidden, ca_coords, antibody_mask, antigen_mask
        )
        ...
        trsl_body = torch.matmul(
            (trsl_xt_physical - antigen_com).unsqueeze(1), rota_xt
        ).squeeze(1)
        ...
        pred_trsl_body = (
            c_skip.unsqueeze(-1) * trsl_body
            + c_out.unsqueeze(-1) * pred_trsl_residual
        )
        pred_trsl_global = antigen_com + torch.matmul(
            pred_trsl_body.unsqueeze(1), rota_xt.transpose(-1, -2)
        ).squeeze(1)

        pred_rota_vec_norm = self.rota_head(rota_input)
        pred_delta_rota = self._so3_exp_map(
            (rota_rms.unsqueeze(-1) * pred_rota_vec_norm).float()
        ).to(dtype)
        pred_rota_global = rota_xt @ pred_delta_rota

        updated_coords = curr_coords.clone()
        for b in range(batch_size):
            antibody_indices = torch.nonzero(antibody_mask[b], as_tuple=False).squeeze(-1)
            moved = local_to_global_coords(
                antibody_local_coords[b].to(pred_rota_global),
                pred_rota_global[b],
                pred_trsl_global[b],
            )
            compressed_fr_mask = fr_mask[b, antibody_indices]
            updated_coords[b, antibody_indices[compressed_fr_mask]] = moved[compressed_fr_mask].to(updated_coords)
