...
class CDRLoopHead(nn.Module):
    ...
    def __init__(
        ...
        # xt_scaled (c_in scale) + token_cond + atom_mask
    ...
    def forward(
        ...
        loop_sfea_init: torch.Tensor,            # [B, N_loop, L_max, c_s]
        ...
        token_cond_init = self.single_cond_init(
            torch.cat([loop_sfea_init, loop_encd, type_feat, pos_feat], dim=-1)
        )

        token_cond = token_cond + token_cond_init
        ...
        # ===== residual refinement: x0_{l+1} = x0_l + delta (zero-init => identity) =====
        delta = self.coord_head(atom_hidden).view(bsz, n_loop, lmax, self.n_atom, 3)
        F_theta = loop_xt_scaled + delta
        F_theta = F_theta * loop_atom_valid_mask.unsqueeze(-1).to(F_theta.dtype)
